        ...
        this.each(function() {
            ...
            $self.one("appear", function() {
                if (!this.loaded) {
                    ...
                    var original = "";
                    if (settings.data_attribute.indexOf('|') > 0) {
                        for (var i = 0; i < settings.data_attribute.split('|').length; i++) {
                            if ($self.attr("data-" + settings.data_attribute.split('|')[i]) != undefined) {
                                original = $self.attr("data-" + settings.data_attribute.split('|')[i]);
                            }
                        }
                    } else {
                        original = settings.data_attribute;
                    }
...
function callLazy() {
    $('img.prodimg[data-src]').lazyload({
        load: function() {
            this.style.opacity = 1;
        }
    });
    $('img.lazy[data-src],.lb-lazy').lazyload({
        load: function() {
            this.style.opacity = 1;
        }
    });
...
function callSlider(inp, isShowDot, autoPlay = false, navigation = true) {
    inp.owlCarousel({
        autoPlay: autoPlay,
        items: 6,
        itemsDesktop: [1199, 5],
...
function callSlider2(inp, isShowDot, autoPlay = false, navigation = true) {
    inp.owlCarousel({
        autoPlay: autoPlay,
        items: 4,
        itemsDesktop: [1199, 4],
...
function corebrain() {
    var t, n, i;
    document.showCoreBrain != undefined && document.showCoreBrain && $("header .cart").length > 0 && monster.get("TMP_CBCart") != undefined && (t = monster.get("TMP_CBCart"), n = JSON.parse(t), n != null && Object.keys(n).length > 0 && (i = Object.keys(n.BasicData).length, $("#cbCount").text(i), i > 0 ? ($("header .cart").show(), $.get("/webapi/corebrain/get", {
        data: t,
        ...
        monster.set("TMP_CBCart", n["<Json>k__BackingField"], 7)
    })) : $("header .cart").hide()), $("#cart-box").hover(function() {
...
(document.cookie.match(/TMP_CBCart/g) || []).length > 1 && (clear = "TMP_CBCart=; expires=Fri, 31 Dec 1999 23:59:59 GMT;", document.cookie = clear, document.cookie = clear + " domain=." + location.host + ";");
